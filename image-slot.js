...
(() => {
  // Per-page sidecar: each HTML file gets its own <slug>.state.json so the
  // shared project-root file can't grow past the host writeFile size cap as
  // slots fill across many pages. The host allowlist only permits a single
  // dot-token before .state.json (no spaces/extra dots), so the document name
  // is slugified to [a-z0-9-]. Falls back to the legacy shared file.
  const STATE_FILE = (() => {
    try {
      const slug = decodeURIComponent((location.pathname.split('/').pop() || ''))
        ...
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '');
      return slug ? slug + '.state.json' : 'image-slots.state.json';
    } catch (e) { return '.image-slots.state.json'; }
  })();
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      const cap = Math.min(MAX_DIM, Math.max(900, Math.round(targetW * 3)) || MAX_DIM);
